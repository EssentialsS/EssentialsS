...
public class UnloadedWorldAdapterImpl extends AbstractMessageAdapter implements UnloadedWorldMessageAdapter {

    private static final SingleDefaultConfigValueWrapper<Component> CONFIG_VALUE;

    static {
        ComponentConfigValue messageConfigValue = new ComponentConfigValue("world", "unload", "Unloaded");
        Component defaultMessage = Component.text("Created world of " + SPlaceHolders.RESOURCE_KEY.copyWithTagType("world").formattedPlaceholderTag() + ".");
        CONFIG_VALUE = new SingleDefaultConfigValueWrapper<>(messageConfigValue, defaultMessage);
    }

    public UnloadedWorldAdapterImpl() {
        super(CONFIG_VALUE);
    }
