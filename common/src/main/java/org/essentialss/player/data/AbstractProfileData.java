...
public abstract class AbstractProfileData implements SGeneralUnloadedData {
    ...
    AbstractProfileData() {
        ...
        this.hasGodMode.bindTo(this.immuneTo, immuneTo -> {
            ...
            DamageType outOfWorld = OrElse.ifTry(Exception.class, () -> {
                Supplier<DamageType> type = (Supplier<DamageType>) DamageTypes.class.getField("VOID").get(null);
                return type.get();
            }, () -> {
                Supplier<DamageType> type = null;
                try {
                    type = (Supplier<DamageType>) DamageTypes.class.getField("OUT_OF_WORLD").get(null);
                } catch (IllegalAccessException | NoSuchFieldException e) {
                    throw new RuntimeException("Sponge API not likely supported", e);
                }
                return type.get();
            });

            types.remove(outOfWorld);
