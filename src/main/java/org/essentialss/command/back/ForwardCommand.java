...
public class ForwardCommand {
    ...
    public static CommandResult execute(SGeneralPlayerData player, int spaces, @Nullable SWorldData targetWorld) {
        ...
        if (opIndex.isPresent()) {
            currentLocations = currentLocations.subList(opIndex.getAsInt(), currentLocations.size());
        }

        if (currentLocations.size() < spaces) {
            return CommandResult.error(Component.text("Cannot go forward that far"));
        }
        OfflineLocation loc = currentLocations.get(spaces - 1);
