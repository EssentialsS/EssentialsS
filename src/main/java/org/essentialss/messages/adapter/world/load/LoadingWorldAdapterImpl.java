package org.essentialss.messages.adapter.world.load;
...
public class LoadingWorldAdapterImpl extends AbstractMessageAdapter implements LoadingWorldMessageAdapter {

    private static final SingleDefaultConfigValueWrapper<Component> CONFIG_VALUE;

    static {
        ComponentConfigValue messageConfigValue = new ComponentConfigValue("world", "load", "Loading");
        Component defaultMessage = Component.text("Loading world of " + SPlaceHolders.RESOURCE_KEY.copyWithTagType("world").formattedPlaceholderTag() + ".");
        CONFIG_VALUE = new SingleDefaultConfigValueWrapper<>(messageConfigValue, defaultMessage);
    }

    public LoadingWorldAdapterImpl() {
        super(CONFIG_VALUE);
    }
